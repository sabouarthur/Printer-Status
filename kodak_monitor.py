...
def run_gui(mon):
    ...
    screensaver_enabled = tk.BooleanVar(master=root, value=bool(gui_cfg.get("screensaver_enabled", False)))
    screensaver_timeout_min = tk.IntVar(master=root, value=int(gui_cfg.get("screensaver_timeout_minutes", 5)))
    screensaver_pin_var = tk.StringVar(master=root, value="")
    veille_wins = []  # une fenêtre de verrouillage par écran physique (écran de veille)
    ...
    # === ÉCRAN DE VEILLE (verrouillage par PIN après inactivité) ===
    def _note_activity(event=None):
        last_activity[0] = time.time()

    root.bind_all("<Motion>", _note_activity, add="+")
    ...
    def _enum_monitor_rects():
        ...
        try:
            ...
            u32.EnumDisplayMonitors(0, 0, MonitorEnumProc(_callback), 0)
        except Exception as e:
            log_print(f"⚠ Énumération écrans impossible: {e}")
        if not rects:
            rects = [(0, 0, root.winfo_screenwidth(), root.winfo_screenheight())]
        return rects

    def _set_taskbars_visible(visible):
        """Masque/affiche la barre des tâches principale + celles des écrans secondaires."""
        try:
            u32 = ctypes.windll.user32
            SW_SHOW, SW_HIDE = 5, 0
            ...
            for cls in ("Shell_TrayWnd", "Shell_SecondaryTrayWnd"):
                ...
                while hwnd:
                    ...
                    hwnd = u32.FindWindowExW(None, hwnd, cls, None)
        except Exception as e:
            log_print(f"⚠ Masquage barre des tâches impossible: {e}")

    atexit.register(lambda: _set_taskbars_visible(True))  # filet de sécurité si l'app se ferme verrouillée

    def show_veille_lock():
        if veille_wins and any(w.winfo_exists() for w in veille_wins):
            return
        _set_taskbars_visible(False)
        veille_wins.clear()

        # Variables partagées : la saisie sur un écran est recopiée sur tous les autres
        veille_pin_var = tk.StringVar(master=root, value="")
        veille_info_var = tk.StringVar(master=root, value="")
        entries = []

        def unlock(event=None):
            if _verify_config_pin(veille_pin_var.get(), gui_cfg.get("screensaver_pin_hash", "")):
                last_activity[0] = time.time()
                for w in list(veille_wins):
                    try:
                        w.destroy()
                    except Exception:
                        pass
                veille_wins.clear()
                _set_taskbars_visible(True)
            else:
                veille_info_var.set("Code PIN incorrect.")
                for w in veille_wins:
                    try:
                        w.bell()
                    except Exception:
                        pass
        ...
        def append_digit(d):
            current = veille_pin_var.get()
            if len(current) < 12:
                veille_pin_var.set(current + str(d))
            if entries:
                entries[0].icursor(tk.END)
                entries[0].focus_force()

        def backspace_digit():
            veille_pin_var.set(veille_pin_var.get()[:-1])
            if entries:
                entries[0].focus_force()
        ...
        def _build_one(rect):
            ...
            vw = tk.Toplevel(root)
            vw.overrideredirect(True)
            vw.configure(bg="#05070a")
            vw.geometry(f"{w}x{h}+{left}+{top}")
            vw.attributes("-topmost", True)
            vw.protocol("WM_DELETE_WINDOW", lambda: None)  # pas de fermeture par la croix/Alt+F4
            ...
            def _keep_on_top():
                if vw.winfo_exists():
                    try:
                        ...
                        pass
                    vw.after(250, _keep_on_top)
            vw.after(250, _keep_on_top)
            vw.bind("<FocusOut>", lambda e: vw.after(50, lambda: (vw.lift(), vw.focus_force())))

            frm = tk.Frame(vw, bg="#05070a")
            frm.place(relx=0.5, rely=0.5, anchor="center")

            tk.Label(frm, text="🔒", font=("Segoe UI", 56), bg="#05070a", fg=TD).pack(pady=(0, 10))
            tk.Label(frm, text="Écran verrouillé", font=("Segoe UI", 22, "bold"), bg="#05070a", fg=TX).pack()
            tk.Label(frm, text="Saisissez le code PIN pour déverrouiller", font=("Segoe UI", 12),
                     bg="#05070a", fg=TD).pack(pady=(4, 20))

            entry = tk.Entry(frm, textvariable=veille_pin_var, show="●", font=("Segoe UI", 28, "bold"),
                              bg=BC, fg=TX, insertbackground=TX, relief="flat", justify="center", width=10)
            entry.pack(pady=(0, 12), ipady=12)
            entries.append(entry)

            tk.Label(frm, textvariable=veille_info_var, font=("Segoe UI", 11, "bold"), bg="#05070a", fg=RD).pack(pady=(0, 6))

            keypad = tk.Frame(frm, bg="#05070a")
            keypad.pack(pady=(0, 4))
            keypad_layout = [
                [("1", lambda: append_digit(1)), ("2", lambda: append_digit(2)), ("3", lambda: append_digit(3))],
                [("4", lambda: append_digit(4)), ("5", lambda: append_digit(5)), ("6", lambda: append_digit(6))],
                [("7", lambda: append_digit(7)), ("8", lambda: append_digit(8)), ("9", lambda: append_digit(9))],
            ...
            for r, row in enumerate(keypad_layout):
                for c, (caption, cmd) in enumerate(row):
                    is_action = caption in {"Effacer", "⌫"}
                    tk.Button(keypad, text=caption, command=cmd, width=9 if is_action else 7, height=3,
                              font=("Segoe UI", 16, "bold"), bg="#1f2937" if not is_action else "#374151",
                              fg=TX, activebackground="#4b5563", activeforeground=TX,
                              relief="flat", bd=0).grid(row=r, column=c, padx=8, pady=8, sticky="nsew")
            for c in range(3):
                keypad.grid_columnconfigure(c, weight=1)

            tk.Button(frm, text="✓ DÉVERROUILLER", command=unlock, bg="#1a4d2e", fg=GR, relief="flat",
                      padx=24, pady=12, font=("Segoe UI", 13, "bold")).pack(pady=(14, 0))

            vw.bind("<Return>", unlock)
            veille_wins.append(vw)
            return vw

        for rect in _enum_monitor_rects():
            _build_one(rect)
        if entries:
            entries[0].focus_force()
    ...
    def build_config_window():
        ...
        config_access_pin_var.set("")
        screensaver_enabled.set(bool(gui_cfg.get("screensaver_enabled", False)))
        screensaver_timeout_min.set(int(gui_cfg.get("screensaver_timeout_minutes", 5)))
        screensaver_pin_var.set("")
        ...
        nb.add(t7,text="  Écran de veille  ")
        ...
        # === TAB ÉCRAN DE VEILLE ===
        veille_main = tk.Frame(t7, bg=BG, padx=20, pady=16)
        veille_main.pack(fill=tk.BOTH, expand=True)

        veille_box = tk.Frame(veille_main, bg=BC, padx=20, pady=14, highlightbackground=BO, highlightthickness=1)
        veille_box.pack(fill=tk.X)
        tk.Label(veille_box, text="VERROUILLAGE AUTOMATIQUE", font=("Segoe UI", 11, "bold"), bg=BC, fg=AM).pack(anchor="w", pady=(0, 8))
        tk.Checkbutton(veille_box, text="Verrouiller l'écran après une période d'inactivité", variable=screensaver_enabled,
                       bg=BC, fg=TX, selectcolor=BD, activebackground=BC, activeforeground=TX,
                       font=("Segoe UI", 10)).pack(anchor="w")

        veille_timeout_row = tk.Frame(veille_box, bg=BC)
        veille_timeout_row.pack(fill=tk.X, pady=(10, 0))
        tk.Label(veille_timeout_row, text="Délai d'inactivité avant verrouillage :", font=("Segoe UI", 10), bg=BC, fg=TX).pack(side=tk.LEFT)
        tk.Spinbox(veille_timeout_row, from_=1, to=120, textvariable=screensaver_timeout_min, width=5,
                   font=("Consolas", 10), bg=BD, fg=TX, buttonbackground="#1c2333").pack(side=tk.LEFT, padx=(8, 4))
        tk.Label(veille_timeout_row, text="minute(s)", font=("Segoe UI", 9), bg=BC, fg=TD).pack(side=tk.LEFT)

        veille_pin_row = tk.Frame(veille_box, bg=BC)
        veille_pin_row.pack(fill=tk.X, pady=(10, 0))
        tk.Label(veille_pin_row, text="PIN de déverrouillage :", font=("Segoe UI", 10), bg=BC, fg=TX).pack(side=tk.LEFT)
        tk.Entry(veille_pin_row, textvariable=screensaver_pin_var, show="*", width=16, font=("Segoe UI", 10),
                 bg=BD, fg=TX, insertbackground=TX, relief="flat").pack(side=tk.LEFT, padx=(8, 0))
        tk.Label(veille_pin_row, text="laisser vide pour conserver le PIN actuel", font=("Segoe UI", 9), bg=BC, fg=TD).pack(side=tk.LEFT, padx=(8, 0))

        tk.Label(
            veille_box,
            text="Ce PIN est propre à l'écran de veille : il est différent du PIN qui\n"
                 "protège l'accès aux paramètres (ci-dessus). Un clavier numérique\n"
                 "tactile s'affiche à l'écran : aucun clavier physique requis.",
            font=("Segoe UI", 9), bg=BC, fg=TD, justify="left",
        ).pack(anchor="w", pady=(10, 0))

        veille_state_lbl = tk.Label(veille_box, text="", font=("Segoe UI", 9), bg=BC, fg=TD)
        veille_state_lbl.pack(anchor="w", pady=(10, 0))

        def _veille_state_text():
            pin_set = bool(str(gui_cfg.get("screensaver_pin_hash", "")).strip())
            return (f"Verrouillage auto: {'ACTIF' if screensaver_enabled.get() else 'INACTIF'}  |  "
                    f"Délai: {gui_cfg.get('screensaver_timeout_minutes', 5)} min  |  "
                    f"PIN: {'défini' if pin_set else 'défaut (1234)'}")

        def apply_veille_settings():
            gui_cfg["screensaver_enabled"] = bool(screensaver_enabled.get())
            try:
                gui_cfg["screensaver_timeout_minutes"] = max(1, int(screensaver_timeout_min.get()))
            except (tk.TclError, ValueError):
                screensaver_timeout_min.set(5)
                gui_cfg["screensaver_timeout_minutes"] = 5
            pin_value = str(screensaver_pin_var.get()).strip()
            if pin_value:
                gui_cfg["screensaver_pin_hash"] = _hash_config_pin(pin_value)
                screensaver_pin_var.set("")
            elif gui_cfg["screensaver_enabled"] and not str(gui_cfg.get("screensaver_pin_hash", "")).strip():
                gui_cfg["screensaver_pin_hash"] = _hash_config_pin("1234")
            save_gui_config(gui_cfg)
            last_activity[0] = time.time()  # évite un verrouillage immédiat juste après activation
            veille_state_lbl.config(text=_veille_state_text())

        def test_veille_lock():
            apply_veille_settings()  # enregistre d'abord le PIN/délai éventuellement saisis
            show_veille_lock()

        veille_btn_row = tk.Frame(veille_box, bg=BC)
        veille_btn_row.pack(anchor="w", pady=(10, 0))
        tk.Button(veille_btn_row, text="✓ Valider écran de veille", command=apply_veille_settings, bg="#1a4d2e", fg=GR,
                  font=("Segoe UI", 10, "bold"), relief="flat", padx=14).pack(side=tk.LEFT)
        tk.Button(veille_btn_row, text="▶ TEST", command=test_veille_lock, bg="#1c2333", fg=AM,
                  font=("Segoe UI", 10, "bold"), relief="flat", padx=14).pack(side=tk.LEFT, padx=(10, 0))
        veille_state_lbl.config(text=_veille_state_text())
