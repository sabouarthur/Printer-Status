...
CONFIG_DEFAULTS = {
    ...
    "hotfolder_path": "",
    "hotfolder_printer": "",
    "hotfolder_format": "10x15 sans bordure",
    "hotfolder_copies": 1,
    ...
    try:
        if os.path.exists(GUI_CONFIG_FILE):
            ...
            if isinstance(data, dict):
                ...
                if legacy_keys:
                    log_print(f"[CONFIG] Nettoyage: anciennes cles retirees: {', '.join(legacy_keys)}")
...
class Monitor:
    ...
    def _get_windows_spooler_jobs(self):
        ...
        for printer in printers:
            if isinstance(printer, dict):
                ...
                name = printer[2] if len(printer) > 2 else ""
                driver = printer[3] if len(printer) > 3 else ""
            ...
            try:
                ...
                jobs = win32print.EnumJobs(handle, 0, 32, 1)
                if jobs:
                    active.append({"printer": str(name), "jobs": len(jobs)})
            except Exception:
                continue
            finally:
    ...
    def is_windows_driver_printing(self):
        ...
        if active_jobs:
            ...
            self._windows_spooler_active_until = now + WINDOWS_SPOOLER_GRACE_SECONDS
        ...
        with self._lock:
            ...
            for m,sdk in self.sdks.items():
                ...
                try:
                    ...
                    log_print(f"[SCAN] {m.upper()} pids={pids} codes={list(st.values())}")
                    for pid in pids:
...
class HotFolder:
    ...
    FORMATS = ["10x15 sans bordure"]

    def __init__(self, path, printer, fmt, copies, action, gap_sec, monitor=None):
        self.path = path
        self.printer = printer  # Conserve pour compatibilite GUI; l'impression SDK utilise l'USB 68xx.
        self.fmt = fmt
    ...
    def _print_file(self, fpath):
        if not str(self.fmt).startswith("10x15"):
            raise RuntimeError(f"Format non supporte par l'integration SDK: {self.fmt}")
...
def run_gui(mon):
    ...
    config_access_pin_var = tk.StringVar(master=root, value=str(gui_cfg.get("config_access_pin", "1234") or "1234"))
    config_pin_prompt = [None]

    popup = tk.Frame(root, bg="#111111", padx=10, pady=8)
    popup.pack(fill=tk.BOTH, expand=True)

    status_dot = tk.Label(popup, text="●", font=("Segoe UI", 24), bg="#111111", fg=TD)
    status_dot.pack(side=tk.LEFT, padx=(0, 8), anchor="n")

    status_block = tk.Frame(popup, bg="#111111")
    status_block.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

    status_text = tk.Label(status_block, text="...", font=("Segoe UI", 11, "bold"), bg="#111111", fg=TD, anchor="w", justify="left")
    status_text.pack(anchor="w")
    status_detail = tk.Label(status_block, text="", font=("Segoe UI", 10, "italic"), bg="#111111", fg="#f0883e", anchor="w", justify="left", wraplength=620)
    status_detail.pack(anchor="w")
    version_lbl_popup = tk.Label(status_block, text=f"v{APP_VERSION}", font=("Segoe UI", 7), bg="#111111", fg="#2d3236", anchor="e")
    version_lbl_popup.pack(anchor="e")

    def release_popup_for_config():
        """Laisse la fenêtre de config/PIN passer devant sans exposer le bureau."""
        config_visible[0] = True
        # Ne PAS quitter le fullscreen — on garde le fond rouge visible
        # Juste baisser topmost pour que la fenêtre PIN/config puisse passer devant
        try:
    ...
    def ask_config_pin_then_open():
        ...
        tk.Label(pin_head, text=f"v{APP_VERSION}", font=("Segoe UI", 8), bg=BC, fg=TD).pack(side=tk.RIGHT, anchor="e")
        with mon._lock:
        ...
        def validate_pin(event=None):
            expected = str(config_access_pin_var.get()).strip() or "1234"
            if pin_entry_var.get() == expected:
                try:
                    pw.grab_release()
                except:
                    pass
    ...
    btn_cfg = tk.Button(popup, text="⚙", font=("Segoe UI", 12), bg="#222", fg=AM,
                         relief="flat", padx=6, command=open_config, cursor="hand2")
    btn_cfg.pack(side=tk.LEFT, padx=(6, 0), anchor="n")
    ...
    def save_popup_settings():
        ...
        gui_cfg["config_access_pin"] = pin_value if pin_value else "1234"
    ...
    def apply_popup_mode(display_mode="normal"):
        ...
        if fullscreen:
            ...
            status_block.configure(bg=popup_bg)
            popup.pack_forget()
            popup.place(relx=0.5, rely=0.5, anchor="center")
            status_dot.pack_configure(side=tk.TOP, anchor="center", padx=0, pady=(0, 10))
            status_block.pack_configure(side=tk.TOP, fill=tk.NONE, expand=False)
            status_dot.configure(bg=popup_bg, font=("Segoe UI", fz * 6))
            status_text.configure(bg=popup_bg, font=("Segoe UI", fz * 4, "bold"), anchor="center", justify="center")
            status_text.pack_configure(anchor="center")
            sw = max(800, root.winfo_screenwidth() - 200)
            status_detail.configure(bg=popup_bg, font=("Segoe UI", fz * 3, "bold"), anchor="center", justify="center", wraplength=sw)
            status_detail.pack_configure(anchor="center", pady=(20, 0))
            version_lbl_popup.configure(bg=popup_bg)
            ...
            popup.place_forget()
            popup.pack(fill=tk.BOTH, expand=True)
            status_dot.pack_configure(side=tk.LEFT, anchor="n", padx=(0, 8), pady=0)
            status_block.pack_configure(side=tk.LEFT, fill=tk.BOTH, expand=True)
            status_dot.configure(bg=popup_bg, font=("Segoe UI", fz + 2))
            status_text.configure(bg=popup_bg, font=("Segoe UI", fz, "bold"), anchor="w", justify="left")
            status_text.pack_configure(anchor="w")
            status_detail.configure(bg=popup_bg, font=("Segoe UI", max(8, fz - 1), "italic"), wraplength=max(300, fz * 42), anchor="w", justify="left")
            status_detail.pack_configure(anchor="w", pady=0)
            version_lbl_popup.configure(bg=popup_bg)
            ...
            if not _blink_active:
                ...
                status_dot.configure(font=("Segoe UI", fz + 2))
                status_text.configure(font=("Segoe UI", fz, "bold"), anchor="w", justify="left")
            popup.place_forget()
            popup.pack(fill=tk.BOTH, expand=True)
            status_dot.pack_configure(side=tk.LEFT, anchor="n", padx=(0, 8), pady=0)
            status_block.pack_configure(side=tk.LEFT, fill=tk.BOTH, expand=True)
    ...
    for _w in (popup, status_dot, status_block, status_text, status_detail):
    ...
    def build_config_window():
        ...
        config_access_pin_var.set(str(gui_cfg.get("config_access_pin", "1234") or "1234"))
        ...
        tk.Label(sb_frame, text=f"v{APP_VERSION}", font=("Segoe UI", 8), bg="#0d1117", fg="#2a3a4a").pack(side=tk.RIGHT)
        ...
        t2=tk.Frame(nb,bg=BG); t3=tk.Frame(nb,bg=BG); t4=tk.Frame(nb,bg=BG); t5=tk.Frame(nb,bg=BG)
        nb.add(t1,text="  Statut  ")
        nb.add(t4,text="  Paramètres  ")
        nb.add(t5,text="  Hotfolder  ")
        nb.add(t2,text="  Codes erreur  "); nb.add(t3,text="  Installation  ")
        ...
        tk.Label(pin_row,text="Code PIN :",font=("Segoe UI",10),bg=BC,fg=TX).pack(side=tk.LEFT)
        ...
        def apply_gui_settings():
            ...
            access_state_lbl.config(
                text=f"Protection accès: {'ACTIVE' if protect_config_access.get() else 'INACTIVE'}  |  PIN défini: {'OUI' if str(config_access_pin_var.get()).strip() else 'NON'}"
            )
            apply_popup_mode(_current_mode())

        tk.Button(popup_opts,text="✓ Valider",command=apply_gui_settings,bg="#1a4d2e",fg=GR,
                  font=("Segoe UI",10,"bold"),relief="flat",padx=14).pack(anchor="w",pady=(8,0))
        ...
        tk.Label(pr_row,text="Imprimante :",font=("Segoe UI",10),bg=BC,fg=TD,width=22,anchor="w").pack(side=tk.LEFT)
        try:
        ...
        # Format
        fmt_row=tk.Frame(hf_main,bg=BC); fmt_row.pack(fill=tk.X,pady=2)
        tk.Label(fmt_row,text="Format d'impression :",font=("Segoe UI",10),bg=BC,fg=TD,width=22,anchor="w").pack(side=tk.LEFT)
        hf_fmt_var=tk.StringVar(value=gui_cfg.get("hotfolder_format","10x15 sans bordure"))
        ttk.Combobox(fmt_row,textvariable=hf_fmt_var,values=HotFolder.FORMATS,width=24,state="readonly",
                     font=("Segoe UI",10)).pack(side=tk.LEFT)
        ...
        def save_hotfolder():
            ...
            gui_cfg["hotfolder_format"]     = hf_fmt_var.get()
            ...
            if p and os.path.isdir(p):
                hotfolder[0] = HotFolder(p, gui_cfg["hotfolder_printer"],
                                         gui_cfg["hotfolder_format"], gui_cfg["hotfolder_copies"],
        ...
        tk.Label(hf_main,
                 text="Formats supportés : JPG, JPEG, PNG, TIF, TIFF, BMP\n"
    ...
    def refresh_popup():
        ...
        if _should_blink:
            ...
            status_dot.configure(bg=bg, fg=fg)
            status_text.configure(fg=fg, bg=bg)
            status_detail.configure(fg=fg, bg=bg)
            btn_cfg.configure(bg=bg)
    ...
    def _blink_tick():
        if refresh_popup._blink_active:
            ...
            if refresh_popup._blink_on:
                c_bg = "#ff0040"; c_fg = "#ffffff"
            else:
                c_bg = "#1f0808"; c_fg = "#ff5555"
            root.configure(bg=c_bg)
            popup.configure(bg=c_bg)
            status_block.configure(bg=c_bg)
            status_dot.configure(bg=c_bg, fg=c_fg)
            status_text.configure(bg=c_bg, fg=c_fg)
            status_detail.configure(bg=c_bg, fg=c_fg)
            btn_cfg.configure(bg=c_bg)
            version_lbl_popup.configure(bg=c_bg)
    ...
    if _hf_path and os.path.isdir(_hf_path):
        hotfolder[0] = HotFolder(
            _hf_path,
            gui_cfg.get("hotfolder_printer", ""),
            gui_cfg.get("hotfolder_format", "10x15 sans bordure"),
